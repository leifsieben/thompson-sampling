...
class ThompsonSampler:
    ...
    def evaluate_batch(self, batch_choice_lists: List[List[int]]) -> List[Tuple[str, str, float]]:
        """Evaluate a batch of reagent combinations at once.
        :param batch_choice_lists: List of choice_lists, each specifying a reagent combination
        :return: List of (product_smiles, product_name, score)
        """
        results = []
        reagent_score_map = defaultdict(list)

        for choice_list in batch_choice_lists:
            selected_reagents = []
            for idx, choice in enumerate(choice_list):
                component_reagent_list = self.reagent_lists[idx]
                selected_reagents.append(component_reagent_list[choice])
            ...
            if prod:
                ...
                if np.isfinite(res):
                    for reagent in selected_reagents:
                        reagent_score_map[reagent].append(res)
            
            results.append((product_smiles, product_name, res))

        # Perform batch updates for each reagent
        for reagent, scores in reagent_score_map.items():
            reagent.batch_update(scores)
    ...
    def search(self, num_cycles=25, batch_size=1):
        """Run the search with batch processing
        :param num_cycles: number of search iterations
        :param batch_size: number of molecules to evaluate per batch (default 1 for original behavior)
        :return: a list of SMILES and scores
        """
        out_list = []
        rng = np.random.default_rng()
        batch_choices = []  # Collect choices for batch evaluation
        ...
        for i in tqdm(range(0, num_cycles), desc="Cycle", disable=self.hide_progress):
            ...
            batch_choices.append(selected_reagents.copy())

            # Process batch if we've collected enough molecules
            if len(batch_choices) >= batch_size:
                batch_results = self.evaluate_batch(batch_choices)  # Call evaluate_batch()
                out_list.extend(batch_results)  # Store results
                batch_choices = []  # Reset batch buffer
            ...
            if i % 100 == 0:
                ...
                if isinstance(top_score, (int, float)):
                    ...
                    self.logger.info(f"Iteration: {i} max score: N/A smiles: {top_smiles} {top_name}")

        # Process any remaining molecules in the last batch
        if batch_choices:
            batch_results = self.evaluate_batch(batch_choices)
            out_list.extend(batch_results)

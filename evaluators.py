...
class BatchFPEvaluator(Evaluator):
    """A batch evaluator that calculates fingerprint Tanimoto similarity to a reference molecule."""

    def __init__(self, input_dict):
        """
        Initializes the evaluator with a reference molecule.
        :param input_dict: Dictionary with key "query_smiles" (SMILES string of reference molecule).
        """
        self.ref_smiles = input_dict["query_smiles"]
        self.fpgen = rdFingerprintGenerator.GetMorganGenerator()
        self.ref_mol = Chem.MolFromSmiles(self.ref_smiles)
    ...
    def evaluate(self, mols):
        """
        Evaluates a batch of molecules and computes Tanimoto similarity to the reference molecule.
        :param mols: A list of RDKit molecules.
        :return: List of Tanimoto similarities.
        """
        if not isinstance(mols, list):  # Handle single-molecule case
            mols = [mols]
            single_input = True
        else:
            single_input = False
        ...
        fps = [self.fpgen.GetFingerprint(mol) if mol is not None else None for mol in mols]

        # Compute Tanimoto similarities
        scores = [DataStructs.TanimotoSimilarity(self.ref_fp, fp) if fp is not None else 0.0 for fp in fps]

        self.num_evaluations += len(mols)

        return scores[0] if single_input else scores

class BatchMWEvaluator(Evaluator):
    """Batch evaluator that calculates molecular weight for a batch of molecules."""
    ...
    def evaluate(self, mols):
        """
        Takes a single RDKit molecule or a batch and returns their molecular weights.
        If a SMILES string is provided, it converts it to an RDKit molecule.
        
        :param mols: RDKit molecule, SMILES string, or a list of them.
        :return: Single float (if single molecule) or list of molecular weights.
        """
        single_input = False
        if not isinstance(mols, list):  # Handle single-molecule case
            mols = [mols]
        ...
        processed_mols = []
        for mol in mols:
            if isinstance(mol, str):  # Convert SMILES to molecule
                mol = Chem.MolFromSmiles(mol)
            processed_mols.append(mol)

        self.num_evaluations += len(processed_mols)
        results = [uru.MolWt(mol) if mol is not None else np.nan for mol in processed_mols]

        # Debugging: Print the evaluated scores
        #print(f"DEBUG: Batch Evaluator - Evaluating {len(processed_mols)} molecules. Scores: {results}")

        return results[0] if single_input else results  # Return single float if input was single

...
class Reagent:
    ...
    def add_score(self, score: float):
        ...
        if self.current_phase == "search":
            ...
            new_mean = self._update_mean(current_var=current_var, observed_value=score)
            new_std = self._update_std(current_var=current_var)

            #print(f"DEBUG: Updating reagent {self.reagent_name} - Old mean: {self.current_mean}, New mean: {new_mean}")
    ...
    def batch_update(self, scores: list):
        """Perform Bayesian updates using a batch of scores.
        
        :param scores: List of observed scores for this reagent
        """
        if self.known_var is None:
            raise ValueError(f"Reagent {self.reagent_name} must be initialized with init_given_prior() before batch updates.")
